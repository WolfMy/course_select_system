...
class Teacher(UserMixin, db.Model):
    ...
    Students = db.relationship(
        "Student", secondary="course_select_table", backref="teacher", lazy="dynamic"
    )
    Courses = db.relationship(
        "Course", secondary="course_teacher", backref="teacher", lazy="dynamic"
    )

    def __init__(
        self, TeacherNum, DeptNum, TeacherName, TeacherSex, TeacherInyear, TeacherTitle
    ):
        self.TeacherNum = TeacherNum
...
class Student(UserMixin, db.Model):
    ...
    Courses = db.relationship(
        "Course", secondary="course_select_table", backref="student", lazy="dynamic"
    )
...
class Course(db.Model):
    ...
    Teachers = db.relationship(
        "Teacher", secondary="course_teacher", backref="course", lazy="dynamic"
    )
    DeptNum = db.Column(db.String(4), db.ForeignKey("dept.DeptNum"), nullable=False)

    def __init__(
        self, CourseNum, CourseName, CourseCredit, CourseTime, DeptNum, CourseDesc
    ):
        self.CourseNum = CourseNum
