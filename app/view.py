...
def student_info(change):
    ...
    return render_template(
        ...
        change=change,
        bg_section_class="bg-section-welcome",
    )
    ...
    if isinstance(current_user._get_current_object(), Student):
        ...
        for Course_ in Courses:
            course_select_table = Course_select_table.query.filter_by(
                StudentNum=current_user.StudentNum, CourseNum=Course_.CourseNum
            ).first()
            teacher = Teacher.query.filter_by(
                TeacherNum=course_select_table.TeacherNum
            ).first()
            table = {
...
def course_teachers(CourseNum):
    if isinstance(current_user._get_current_object(), Student):
        ...
        for course_teacher in course_teachers:
            course_select_table = Course_select_table.query.filter_by(
                CourseNum=CourseNum, TeacherNum=course_teacher.TeacherNum
            ).all()
            teacher = Teacher.query.filter_by(
...
def course():
    if isinstance(current_user._get_current_object(), Student):
        ...
        for course in all_courses:
            ...
            tables.append(table)
        return render_template(
            "student/course.html", tables=tables, course_selected=course_selected
        )


@app.route(
    "/course_drop/<CourseNum>",
    methods=[
        "GET",
    ],
)
...
@app.route(
    "/course_select/<CourseNum>/<TeacherNum>",
    methods=[
        "GET",
    ],
)
...
def course_select(CourseNum, TeacherNum):
    if isinstance(current_user._get_current_object(), Student):
        ...
        if CourseNum in course_selected:
            ...
            course_select = Course_select_table(
                current_user.StudentNum, CourseNum, TeacherNum
            )
    ...
    if isinstance(current_user._get_current_object(), Teacher):
        ...
        for course in courses:
            course_select_tables = Course_select_table.query.filter_by(
                CourseNum=course.CourseNum, TeacherNum=current_user.TeacherNum
            ).all()
            course_info = {
                "CourseNum": course.CourseNum,
                "CourseName": course.CourseName,
            ...
            for course_select_table in course_select_tables:
                student = Student.query.filter_by(
                    StudentNum=course_select_table.StudentNum
                ).first()
                table = {
        ...
        return render_template(
            "teacher/course_select_detail.html", course_tables=course_tables
        )
...
def course_grade_input(CourseNum):
    if isinstance(current_user._get_current_object(), Teacher):
        ...
        return render_template(
            "teacher/course_grade_input.html", course_tables=course_tables
        )
...
def grade_set_zero(CourseNum, StudentNum):
    if isinstance(current_user._get_current_object(), Teacher):
        course_select_table = Course_select_table.query.filter_by(
            StudentNum=StudentNum, CourseNum=CourseNum
        ).first()
...
def student_manage():
    if isinstance(current_user._get_current_object(), Manager):
        ...
        return render_template(
            "admin/student_manage.html", info=info, students=students
        )
    ...
    if isinstance(current_user._get_current_object(), Manager):
        if request.method == "POST":
            ...
            if not Student.query.filter_by(StudentNum=StudentNum).first():
                student = Student(
                    StudentNum, MajorNum, StudentName, StudentSex, StudentInyear
                )
...
def teacher_manage():
    if isinstance(current_user._get_current_object(), Manager):
        ...
        return render_template(
            "admin/teacher_manage.html", info=info, teachers=teachers
        )
    ...
    if isinstance(current_user._get_current_object(), Manager):
        if request.method == "POST":
            ...
            if not Course.query.filter_by(CourseNum=CourseNum).first():
                course = Course(
                    CourseNum, CourseName, CourseCredit, CourseTime, DeptNum, CourseDesc
                )
...
def add_course_teacher():
    if isinstance(current_user._get_current_object(), Manager):
        if request.method == "POST":
            ...
            TeacherName = request.form["TeacherName"]
            TeacherNum = (
                Teacher.query.filter_by(TeacherName=TeacherName).first().TeacherNum
            )
            CourseCapacity = request.form["CourseCapacity"]
            if not Course_Teacher.query.filter_by(
                CourseNum=CourseNum, TeacherNum=TeacherNum
            ).first():
                ...
                flash(
                    '%s老师已开设"%s"课程，请勿重复添加！' % (TeacherName, CourseName)
                )
        return redirect(url_for("course_manage"))
...
def course_select_manage():
    if isinstance(current_user._get_current_object(), Manager):
        ...
        for course_teacher in course_teachers:
            ...
            course_select_tables = Course_select_table.query.filter_by(
                CourseNum=course.CourseNum, TeacherNum=teacher.TeacherNum
            ).all()
            table = {
                "CourseNum": course.CourseNum,
                "CourseName": course.CourseName,
...
def add_course_select():
    if isinstance(current_user._get_current_object(), Manager):
        if request.method == "POST":
            ...
            TeacherNum = request.form["TeacherNum"]
            StudentNum = request.form["StudentNum"]
            if not Course_select_table.query.filter_by(
                CourseNum=CourseNum, StudentNum=StudentNum
            ).first():
                course_select_table = Course_select_table(
                    StudentNum, CourseNum, TeacherNum
                )
...
def drop_course_select():
    if isinstance(current_user._get_current_object(), Manager):
        if request.method == "POST":
            ...
            StudentNum = request.form["StudentNum"]
            course_select_table = Course_select_table.query.filter_by(
                CourseNum=CourseNum, TeacherNum=TeacherNum, StudentNum=StudentNum
            ).first()
